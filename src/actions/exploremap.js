...
export function getGeoDataInfo(datasetId, geo) {
  return (dispatch, state) => {
    ...
    fetch(`${process.env.REACT_APP_RW_API_URL}/query/${datasetId}?sql=SELECT * FROM ${datasetTableName} WHERE ST_INTERSECTS(ST_SetSRID(ST_GeomFromGeoJSON('${geoString}'), 4326), the_geom)`, {
      headers: {
