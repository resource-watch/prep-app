...
export function getDatasetLayer(dataset) {
  return (dispatch) => {
    fetch(`${process.env.REACT_APP_RW_API_URL}/layer/${dataset.layer[0].id}`, {
      headers: {
...
export function getDatasets(defaultActiveLayers) {
  return (dispatch) => {
    const env = process.env.REACT_APP_DATASET_ENV || 'production';
    fetch(`${process.env.REACT_APP_RW_API_URL}/dataset?application=prep&includes=metadata,layer,vocabulary&page[size]=999&status=saved&env=${env}&published=true`, {
      headers: {
        'Content-Type': 'application/json',
        'Upgrade-Insecure-Requests': 1
      }
...
export function getDatasetByIdOrSlug(datasetIdentifier, includesData) {
  ...
  return (dispatch) => {
    const env = process.env.REACT_APP_DATASET_ENV || 'production';
    fetch(`${process.env.REACT_APP_RW_API_URL}/dataset/${datasetIdentifier}?application=prep${includeQuery}&status=saved&page[size]=999&env=${env}`, {
      headers: {
        'Content-Type': 'application/json',
        'Upgrade-Insecure-Requests': 1
      }
...
export function getDatasetDefaultWidget(datasetId) {
  return (dispatch) => {
    fetch(`${process.env.REACT_APP_RW_API_URL}/widget?application=prep&default=true&dataset=${datasetId}`, {
      headers: {
        'Content-Type': 'application/json',
        'Upgrade-Insecure-Requests': 1
      }
      ...
      .then((data) => {
        if (data.data.length) {
          for (let i = 0, wLength = data.data.length; i < wLength; i++) {
            fetch(`${process.env.REACT_APP_RW_API_URL}/widget/${data.data[i].id}`)
              .then((response) => {
                if (response.ok) return response.json();
                throw new Error(response.statusText);
              })
...
export function getDatasetMetadata(datasetId) {
  return (dispatch) => {
    fetch(`${process.env.REACT_APP_RW_API_URL}/metadata/${datasetId}`, {
      headers: {
        'Content-Type': 'application/json',
        'Upgrade-Insecure-Requests': 1
      }
