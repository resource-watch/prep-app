...
export const fetchAdditionalDataset = createThunkAction('fetchAdditionalDataset', (datasetIds) => {
  ...
  const promises = Promise.all(
    datasetIds.map(
      (datasetId) => fetch(`${process.env.REACT_APP_RW_API_URL}/dataset/${datasetId}?${qs.stringify(params)}`, {
        headers: {
...
export const fetchDataset = createThunkAction('fetchDataset', (datasetId) => {
  ...
  return (dispatch) => {
    fetch(`${process.env.REACT_APP_RW_API_URL}/dataset/${datasetId}?${qs.stringify(params)}`, {
      headers: {
        'Content-Type': 'application/json',
        'Upgrade-Insecure-Requests': 1
      }
