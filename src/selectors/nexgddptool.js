  ...
  (layers, mapMode, range1Selection, range2Selection) => { // eslint-disable-line no-shadow
    ...
    if (mapMode !== 'difference') {
      ...
      if (currentLayer && range1Selection) {
        ...
        activeLayers.push({
          url: `${process.env.REACT_APP_RW_API_URL}/layer/${currentLayer.id}/tile/nexgddp/{z}/{x}/{y}?year=${range1Date}`,
          date: range1Date
        });
      }
      ...
      if (currentLayer && range2Selection) {
        ...
        activeLayers.push({
          url: `${process.env.REACT_APP_RW_API_URL}/layer/${currentLayer.id}/tile/nexgddp/{z}/{x}/{y}?year=${range2Date}`,
          date: range2Date
        });
      }
    } else {
      ...
      if (currentLayer && range1Selection && range2Selection) {
        const range1Date = `${range1Selection.value}`;
        const range2Date = `${range2Selection.value}`;
        activeLayers.push({ url: `${process.env.REACT_APP_RW_API_URL}/layer/${currentLayer.id}/tile/nexgddp/{z}/{x}/{y}?year=${range1Date}&compareYear=${range2Date}` });
      }
    }
