  ...
  try {
    ...
    stage ('Build docker') {
      switch ("${env.BRANCH_NAME}") {
        // Roll out to staging
        case "develop":
          sh("docker -H :2375 build -t ${imageTag} --build-arg datasetEnv=production,preproduction --build-arg rwApiUrl=https://staging-api.globalforestwatch.org/v1 --build-arg apiUrl=https://staging.prepdata.org/api --build-arg basemapUrl=https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}.png --build-arg callbackUrl=https://staging.prepdata.org/auth .")
          break
        case "preproduction":
        ...
        case "master":
          ...
          sh("docker -H :2375 build -t ${dockerUsername}/${appName}:latest --build-arg datasetEnv=production --build-arg apiUrl=https://prepdata.org/api .")
          break
        case "preproduction":
          ...
          break
        case "master":
          sh("docker -H :2375 build -t ${imageTag} --build-arg datasetEnv=production --build-arg apiUrl=https://beta.prepdata.org .")
