  ...
  try {
    ...
    stage ('Build docker') {
      switch ("${env.BRANCH_NAME}") {
        // Roll out to staging
        case "develop":
          sh("docker -H :2375 build -t ${imageTag} --build-arg datasetEnv=production,preproduction --build-arg apiUrl=https://www.prepdata.org/api --build-arg basemapUrl=https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}.png --build-arg --build-arg callbackUrl=https://staging.prepdata.org/auth .")
          break
        case "master":
