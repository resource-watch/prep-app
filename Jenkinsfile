  ...
  try {
    ...
    stage ('Build docker') {
      switch ("${env.BRANCH_NAME}") {
        // Roll out to staging
        case "develop":
          sh("docker -H :2375 build -t ${imageTag} --build-arg datasetEnv=production,preproduction --build-arg apiUrl=https://staging.prepdata.org/api --build-arg basemapUrl=https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}.png .")
          sh("docker -H :2375 build -t ${dockerUsername}/${appName}:latest --build-arg datasetEnv=production,preproduction --build-arg apiUrl=https://staging.prepdata.org/api --build-arg basemapUrl=https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}.png .")
          break
        case "preproduction":
          sh("docker -H :2375 build -t ${imageTag} --build-arg datasetEnv=production,preproduction --build-arg apiUrl=https://preproduction.prepdata.org/api --build-arg callbackUrl=https://staging.prepdata.org/auth .")
          sh("docker -H :2375 build -t ${dockerUsername}/${appName}:latest --build-arg datasetEnv=production,preproduction --build-arg apiUrl=https://preproduction.prepdata.org/api --build-arg callbackUrl=https://staging.prepdata.org/auth .")
          break
        case "master":
    ...
    // Notify Success
    slackSend (color: '#00FF00', channel: '#resource-watch-jenkins', message: "SUCCESSFUL: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
    emailext (
    ...
    // Notify Error
    slackSend (color: '#FF0000', channel: '#resource-watch-jenkins', message: "FAILED: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
    emailext (
