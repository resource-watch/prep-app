...
export const updateURLParams = createThunkAction('updateURLParams', () => (dispatch, getState) => {
  const { exploreExportPage } = getState();
  const { tab, datasets, coreDatasets, datasetFilters, map } = exploreExportPage;
  ...
  dispatch(replace({ pathname: '/export/explore', query }));
});
...
export const setInitialFilterStatus = createThunkAction('explore-dataset-filters/setInitialFilterStatus', () => (dispatch, getState) => {
  const { data, originalData, filters } = getState().exploreExportPage.datasetFilters;
...
export const getDatasetsByGraph = createThunkAction('explore-page/getDatasetsByGraph', () =>
  (dispatch, getState) => {
    const { exploreExportPage } = getState();
    const { filters } = exploreExportPage.datasetFilters;
