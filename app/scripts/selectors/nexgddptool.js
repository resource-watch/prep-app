...
const layers = ({ nexgddptool }) => nexgddptool.layers;
...
export const getLayers = createSelector(
  ...
  (layers, range1Selection, range2Selection) => { // eslint-disable-line no-shadow
    ...
    if (range1Selection) {
      ...
      result.push({
        url: `${config.apiUrlRW}/layer/dd272bc7-a5e7-41e2-8ca5-6e3353603fd0/tile/nexgddp/{z}/{x}/{y}?ansi="${range1Date}"`,
        date: range1Date
      });
    }
    ...
    if (range2Selection) {
      ...
      result.push({
        url: `${config.apiUrlRW}/layer/dd272bc7-a5e7-41e2-8ca5-6e3353603fd0/tile/nexgddp/{z}/{x}/{y}?ansi="${range2Date}"`,
        date: range2Date
      });
    }
