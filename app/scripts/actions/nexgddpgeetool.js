...
import { getIndicatorId, getTempResolution } from 'selectors/nexgddpgeetool';
import {
  NEXGDDP_GEE_SET_MAP_ZOOM,
  NEXGDDP_GEE_SET_MAP_CENTER,
  NEXGDDP_GEE_SET_MARKER_MODE,
  NEXGDDP_GEE_SET_MARKER_POSITION,
  NEXGDDP_GEE_SET_MAP_MODE,
  NEXGDDP_GEE_SET_GRAPH_MODE,
  NEXGDDP_GEE_SET_SCENARIO_OPTIONS,
  NEXGDDP_GEE_SET_SCENARIO_SELECTION,
  NEXGDDP_GEE_SET_TEMP_RESOLUTION_OPTIONS,
  NEXGDDP_GEE_SET_TEMP_RESOLUTION_SELECTION,
  NEXGDDP_GEE_SET_RANGE1_OPTIONS,
  NEXGDDP_GEE_SET_RANGE1_SELECTION,
  NEXGDDP_GEE_SET_RANGE2_OPTIONS,
  NEXGDDP_GEE_SET_RANGE2_SELECTION,
  NEXGDDP_GEE_SET_CHART_DATA,
  NEXGDDP_GEE_SET_CHART_LOADED,
  NEXGDDP_GEE_SET_CHART_ERROR,
  NEXGDDP_GEE_SET_BASEMAP,
  NEXGDDP_GEE_SET_LABELS,
  NEXGDDP_GEE_SET_WATER,
  NEXGDDP_GEE_SET_BOUNDARIES,
  NEXGDDP_GEE_SET_RENDER,
  NEXGDDP_GEE_SET_DATASET,
  NEXGDDP_GEE_SET_INDICATOR_DATASET,
  NEXGDDP_GEE_RESET_STATE
} from '../constants';

export function updateUrl() {
  return (dispatch, getState) => {
    const state = getState().nexgddpgeetool;
    const params = {
      zoom: state.map.zoom,
...
export function getChartData() {
  return (dispatch, getState) => {
    ...
    const lat = state.nexgddpgeetool.marker[0];
    const lng = state.nexgddpgeetool.marker[1];
    const indicatorId = getIndicatorId(state);
    const slug = state.nexgddpgeetool.indicatorDataset.slug;
    ...
    return fetch(`${process.env.RW_API_URL}/query?sql=select ${indicatorId}_q25 as q25, ${indicatorId} as q50, ${indicatorId}_q75 as q75, year as x from ${slug}&lat=${lat}&lon=${lng}`, {
      ...
      .catch((err) => {
        ...
        dispatch({
          type: NEXGDDP_GEE_SET_CHART_ERROR,
          payload: true
        });
      })
      .then(() => dispatch({
        type: NEXGDDP_GEE_SET_CHART_LOADED,
        payload: true
      }));
  };
}
...
export function setMarkerPosition(coordinates, changeUrl = true) {
  return (dispatch, getState) => {
    dispatch({
      type: NEXGDDP_GEE_SET_MARKER_POSITION,
      payload: coordinates
    });
    ...
    // location
    if (getState().nexgddpgeetool.indicatorDataset && getState().nexgddpgeetool.marker) {
...
export function setGraphMode(graphMode, changeUrl = true) {
  return (dispatch) => {
    dispatch({
      type: NEXGDDP_GEE_SET_GRAPH_MODE,
      payload: graphMode
    });
...
export function setScenarioOptions(options) {
  return (dispatch) => {
    dispatch({
      type: NEXGDDP_GEE_SET_SCENARIO_OPTIONS,
      payload: options
    });
  };
}
...
export function loadIndicatorDataset() {
  return (dispatch, getState) => {
    const state = getState();
    const indicatorId = getIndicatorId(state);
    const scenario = state.nexgddpgeetool.scenario.selection.value;
    const tempResolution = state.nexgddpgeetool.tempResolution.selection;
    ...
    return fetch(`${process.env.RW_API_URL}/nexgddp/dataset/${indicatorId}/${scenario}/${tempResolution.value}?env=${config.datasetEnv}`, {
      ...
      .then(({ data }) => {
        ...
        if (state.nexgddpgeetool.marker) {
...
export function setScenarioSelection(selection, changeUrl = true) {
  return (dispatch, getState) => {
    dispatch({
      type: NEXGDDP_GEE_SET_SCENARIO_SELECTION,
      payload: selection
    });

    if (getState().nexgddpgeetool.tempResolution.selection) {
...
export function setTempResolutionOptions(options) {
  return (dispatch) => {
    dispatch({
      type: NEXGDDP_GEE_SET_TEMP_RESOLUTION_OPTIONS,
      payload: options
    });
  };
}
...
export function setTempResolutionSelection(selection, changeUrl = true) {
  return (dispatch, getState) => {
    ...
    dispatch({
      type: NEXGDDP_GEE_SET_TEMP_RESOLUTION_SELECTION,
      payload: selection
    });
    ...
    const range1Options = getState().nexgddpgeetool.range1.options;
    ...
    dispatch({
      type: NEXGDDP_GEE_SET_RANGE1_SELECTION,
      payload: selectedOption || range1Options[selection.value][0]
    });

    dispatch({
      type: NEXGDDP_GEE_SET_RANGE2_SELECTION,
      payload: null
    });
...
export function setRange1Options(options) {
  return (dispatch) => {
    dispatch({
      type: NEXGDDP_GEE_SET_RANGE1_OPTIONS,
      payload: options
    });
  };
}
...
export function setRange1Selection(selection, changeUrl = true) {
  return (dispatch, getState) => {
    ...
    const range2Selection = getState().nexgddpgeetool.range2.selection;
...
export function setRange2Options(options) {
  return (dispatch) => {
    dispatch({
      type: NEXGDDP_GEE_SET_RANGE2_OPTIONS,
      payload: options
    });
  };
}
...
export function setBoundaries(boundaries, changeUrl = true) {
  return (dispatch) => {
    dispatch({
      type: NEXGDDP_GEE_SET_BOUNDARIES,
      payload: boundaries
    });
...
export function getUrlState() {
  return (dispatch, getState) => {
    ...
    if (params.scenario) {
      const scenarioOptions = getState().nexgddpgeetool.scenario.options;
    ...
    if (params.tempResolution) {
      const tempResolutionOptions = getState().nexgddpgeetool.tempResolution.options;
      ...
      if (params.range2) {
        const range2Options = getState().nexgddpgeetool.range2.options;
...
export function setDefaultState() {
  return (dispatch, getState) => {
    ...
    if (!store.nexgddpgeetool.scenario.selection && store.nexgddpgeetool.scenario.options.length) {
      promises.push(dispatch(setScenarioSelection(store.nexgddpgeetool.scenario.options[0])));
    }

    if (!store.nexgddpgeetool.range1.selection && Object.keys(store.nexgddpgeetool.range1.options).length && store.nexgddpgeetool.tempResolution.options.length) {
      const options = store.nexgddpgeetool.range1.options[tempResolutionValue];
    ...
    if (!store.nexgddpgeetool.tempResolution.selection && store.nexgddpgeetool.tempResolution.options.length) {
      const tempResolution = store.nexgddpgeetool.tempResolution.options.find(t => t.value === tempResolutionValue);
      promises.push(dispatch(setTempResolutionSelection(tempResolution || store.nexgddpgeetool.tempResolution.options[0])));
    }
...
export function resetState() {
  return dispatch => Promise.resolve(dispatch({ type: NEXGDDP_GEE_RESET_STATE }));
}

export function setDataset(dataset) {
  return dispatch => Promise.resolve(dispatch({
    type: NEXGDDP_GEE_SET_DATASET,
    payload: dataset
  }));
}
